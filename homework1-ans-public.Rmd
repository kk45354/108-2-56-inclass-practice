...
#length：有多少位置可以選
#names：各個位置叫什麼
#factor：當觀察到有固定資料反覆出現時，，可以用Factor改成類別資料
#levels：顯示類別資料當中有哪些資料
#str(example,max.level = n)：總覽資料到n層
----------------------------------------------------------------------------------------------------
...
```{r}
course <- list(
  approve=TRUE, course=list(
    coursename='會計學', type='必', credits=3, time_place=list(
      weekday='星期三', time_slots=c(7,8,9), classroom='商2F05'
    )))
#記得不只一個數字的時候要用vector表示
str(course)
```
